...
class Exchange(ExchangeInterface):
        ...
        try:
            ...
            for action in actions:
                if isinstance(action, AddOrderActionSchema):
                    self._process_add_order(action, account, timestamp)
                elif isinstance(action, CancelOrderActionSchema):
                    self._process_cancel_order(action, account, timestamp)
            ...
            account.account_info.timestamp = timestamp
    ...
    def _process_add_order(self, action: AddOrderActionSchema, account: AccountSchema, timestamp: datetime):
        """处理添加订单操作"""
        try:
            ...
            if order.order_type == OrderType.BUY:
                ...
                if account.account_info.available_cash < total_amount:
                    raise ValueError(f"资金不足，需要{total_amount:.2f}，可用{account.account_info.available_cash:.2f}")
                
                # 冻结资金（在模拟中我们只是检查，不实际冻结）
                self.logger.info(f"买入订单资金检查通过: 需要{total_amount:.2f}，可用{account.account_info.available_cash:.2f}")
                
            else:  # SELL
                # 卖出订单：检查持仓是否足够
                position = self._get_position(account, order.symbol)
    ...
    def _process_cancel_order(self, action: CancelOrderActionSchema, account: AccountSchema, timestamp: datetime):
        """处理取消订单操作"""
        try:
            ...
            order = account.orders[order_index]
            ...
            # 移除订单
            account.orders.pop(order_index)
    ...
    def check_and_execute_orders(self, market: MarketSchema, account: AccountSchema, timestamp: datetime) -> List[OrderResultSchema]:
        ...
        try:
            ...
            # 清理过期订单（超过两周）
            self._cleanup_expired_orders(account, timestamp)
            
            # 检查每个待处理订单
            pending_orders = [order for order in account.orders if order.status == OrderStatus.PENDING]
            ...
            for order in pending_orders:
                ...
                # 尝试执行订单
                result = self._try_execute_order(order, market, account, timestamp)
                if result:
                    executed_results.append(result)
                    self.logger.info(f"订单执行成功: {order.order_id}")
            
            # 更新账户资产数据
            self._update_account_assets(account, market, timestamp)
    ...
    def _cleanup_expired_orders(self, account: AccountSchema, timestamp: datetime):
        """清理过期订单（超过两周）"""
    ...
    def _try_execute_order(self, order: OrderSchema, market: MarketSchema, account: AccountSchema, timestamp: datetime) -> Optional[OrderResultSchema]:
        """尝试执行单个订单"""
        try:
            # 获取股票实时数据
            stock_data = market.stock_data.get(order.symbol)
            if not stock_data:
            ...
            # 检查订单执行条件
            if order.order_type == OrderType.BUY:
                return self._execute_buy_order(order, real_time_data, account, timestamp)
            else:  # SELL
                return self._execute_sell_order(order, real_time_data, account, timestamp)
                
        except Exception as e:
            self.logger.error(f"执行订单失败 {order.order_id}: {str(e)}")
            return None
    
    def _execute_buy_order(self, order: OrderSchema, real_time_data: StockRealTimeData, account: AccountSchema, timestamp: datetime) -> Optional[OrderResultSchema]:
        """执行买入订单"""
        try:
            total_cost = order.price * order.quantity
            commission = total_cost * self.commission_rate
            ...
            # 检查资金是否足够
            if account.account_info.available_cash < total_amount:
                self.logger.warning(f"资金不足，无法执行买入订单: 需要{total_amount:.2f}，可用{account.account_info.available_cash:.2f}")
                return None
            
            # 扣除资金
            account.account_info.available_cash -= total_amount
    ...
    def _execute_sell_order(self, order: OrderSchema, real_time_data: StockRealTimeData, account: AccountSchema, timestamp: datetime) -> Optional[OrderResultSchema]:
        """执行卖出订单"""
        try:
            # 检查持仓是否足够
            position = self._get_position(account, order.symbol)
            ...
            # 增加资金
            account.account_info.available_cash += net_amount
    ...
    def _get_position(self, account: AccountSchema, symbol: str) -> Optional[PositionData]:
        """获取指定股票的持仓"""
    ...
    def _update_account_assets(self, account: AccountSchema, market: MarketSchema, timestamp: datetime):
        """更新账户资产数据"""
        try:
            # 更新持仓的当前价格和市值
            ...
            for position in account.positions:
                # 更新持仓的当前价格（从市场数据获取）
                stock_data = market.stock_data.get(position.symbol)
                if stock_data:
